...
import { selectedCardState, mobileContentToggle } from '@/status/TenMinuteNoteStatus';
...
export default function MobileHome() {
   ...
   const [isKeyword, setIsKeyword] = useRecoilState(mobileContentToggle);
   ...
   return (
      <div>
         ...
         <div className="mt-[20px]">
            {isKeyword ? (
               <>
                  <Keyword text="개발자 협업 : 넥스터즈" isVisible={true} isPc={false}></Keyword>
                  <Keyword text="면접을 준비하는 취업준비생을 위한 서비스" isVisible={true} isPc={false}></Keyword>
                  <Keyword text="개발자와 협업하며 프론트엔드에 매력" isVisible={true} isPc={false}></Keyword>
                  <Keyword
                     text="독학으로 개발을 공부해 6개월 만에 포트폴리오 홈페이지 홈페이지"
                     isVisible={true}
                     isPc={false}></Keyword>
                  <Keyword text="이후 개발자로서 커리어를 쌓기 위해" isVisible={true} isPc={false}></Keyword>
               </>
            ) : (
               <>
                  <p className="text-body2">
                     UX와 UI(User Interface) 디자인 모두 사용자를 위한 최상의 최종 제품을 제공하기 위함이라는 공통분모가
                     있지만 두 분야 사이에는 분명한 차이가 존재합니다.
                     <br></br>
                     <br></br>
                     UX Design
                     <br></br>
                     UX 디자인에는 사용자 조사 및 프로토타이핑에서부터 제품 마케팅에 이르기까지 제품 디자인 및 출시를
                     아울러 고려해야 하는 분석적 프로세스가 포함됩니다. 사용자의 니즈와 비즈니스 또는 브랜드의 니즈
                     사이의 간극 을 메우는 것 이 UX 디자이너의 일이라 할 수 있습니다.
                     <br></br>
                     <br></br>
                     UI Design
                     <br></br>
                     UX가 전체 경험을 구성하는 것이라면 UI(사용자 인터페이스)는 사용자가 상호 작용할 시각적이면서 유형적
                     요소를 만들어 나가는 데 중점을 둡니다. UX UI 디자인의 조화로운 협력을 통해 제품이 개발되어야 하며,
